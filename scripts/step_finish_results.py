"""
Finish results — fast version:
  - Loads all existing cached OOF files
  - Runs only truly missing combos
  - Skips LOO for Full_ensemble (too slow, not critical)
  - Saves both step_10fold_cv_results.csv and step_ablation_results.csv
"""
...
def save_oof(fname, arr):
    np.save(os.path.join(MODEL_DIR, fname), arr)
...
def main():
    ...
    print("\n=== 10-fold CV ===")
    rows_10 = []
    for name, X in combos_10:
        t0 = time.time()
        oof_10 = load_oof(f'oof_10fold_{name}.npy')
        if oof_10 is None:
            print(f"  Running 10fold: {name} ...", flush=True)
            oof_10 = nested_cv(X, y, n_outer=10)
            save_oof(f'oof_10fold_{name}.npy', oof_10)

        # LOO: skip for huge feature sets (>8k dims) — not critical
        skip_loo = X.shape[1] > 8000
        oof_loo = load_oof(f'oof_loo_{name}.npy')
        if oof_loo is None and not skip_loo:
            print(f"  Running LOO:    {name} ...", flush=True)
            oof_loo = nested_loo(X, y)
            save_oof(f'oof_loo_{name}.npy', oof_loo)
        elif oof_loo is None and skip_loo:
            print(f"  Skipping LOO (dim={X.shape[1]}): {name}")
            oof_loo = oof_10  # placeholder so metrics don't crash
        ...
        row = {'combination': name,
               ...
               'ci_lo': lo, 'ci_hi': hi,
               'r_loo': mloo['r'] if not skip_loo else None,
               'rmse_loo': mloo['rmse'] if not skip_loo else None,
               'dim': X.shape[1], 'n': len(y), **st_rs}
        rows_10.append(row)
        loo_str = f"{mloo['r']:.4f}" if not skip_loo else "skip"
        print(f"    {name:<33} R10={m10['r']:.4f}  LOO={loo_str}  [{lo:.3f},{hi:.3f}]  {time.time()-t0:.0f}s")

    pd.DataFrame(rows_10).to_csv(os.path.join(RES_DIR, 'step_10fold_cv_results.csv'), index=False)
    print(f"Saved: step_10fold_cv_results.csv")

    # ── Ablation — priority combos ────────────────────────────────────────────
    modalities = {'PH': X_ph, 'PSRT': X_psrt, 'CPF': X_cpf,
                  'FM': X_fm, 'MFP': X_morgan, 'PHY': X_phys}
    ...
    for name, mod_keys in priority_combos:
        ...
        oof_10 = load_oof(f'oof_ablation_{name}.npy')
        if oof_10 is None:
            print(f"  Running 10fold: {name} ...", flush=True)
            oof_10 = nested_cv(X, y, n_outer=10)
            save_oof(f'oof_ablation_{name}.npy', oof_10)
        else:
            print(f"  [cached] {name}", flush=True)

        oof_loo = load_oof(f'oof_loo_ablation_{name}.npy')
        if oof_loo is None and not skip_loo:
            oof_loo = nested_loo(X, y)
            save_oof(f'oof_loo_ablation_{name}.npy', oof_loo)
        elif oof_loo is None and skip_loo:
            oof_loo = oof_10
        ...
        row = {'combination': name, 'modalities': '+'.join(mod_keys),
               'n_modalities': len(mod_keys), 'dim': X.shape[1],
               'r_10fold': m10['r'], 'rho_10fold': m10['rho'],
               'rmse_10fold': m10['rmse'], 'r2_10fold': m10['r2'],
               'ci_lo': lo, 'ci_hi': hi,
               'r_loo': mloo['r'] if not skip_loo else None,
               ...
               **st_rs}
        rows_ab.append(row)
        loo_s = f"{mloo['r']:.4f}" if not skip_loo else "skip"
        print(f"    R10={m10['r']:.4f}  LOO={loo_s}  [{lo:.3f},{hi:.3f}]  {time.time()-t0:.0f}s")
    ...
    print(f"{'Combination':<35}  {'Dim':>6}  {'R10':>6}  {'RMSE':>6}  {'LOO-R':>6}  {'95%CI':>14}")
    print("-" * 80)
    for r in sorted(rows_ab, key=lambda x: x['r_10fold'], reverse=True):
        ci    = f"[{r['ci_lo']:.3f},{r['ci_hi']:.3f}]"
        loo_s = f"{r['r_loo']:.4f}" if r['r_loo'] is not None else " skip"
        print(f"  {r['combination']:<33}  {r['dim']:>6}  {r['r_10fold']:>6.4f}  "
              f"{r['rmse_10fold']:>6.4f}  {loo_s:>6}  {ci:>14}")

    print("\n" + "=" * 80)
    print("10-FOLD CV TABLE (sorted by R10)")
    print(f"{'Combination':<35}  {'Dim':>6}  {'R10':>6}  {'LOO-R':>6}")
    print("-" * 80)
    for r in sorted(rows_10, key=lambda x: x['r_10fold'], reverse=True):
        loo_s = f"{r['r_loo']:.4f}" if r['r_loo'] is not None else "skip "
        print(f"  {r['combination']:<33}  {r['dim']:>6}  {r['r_10fold']:>6.4f}  {loo_s:>6}")
